...
public class CommentView {
	...
	private void insertComment(int boardNo) {
		...
		// String : 불변성 
			// 메모리에 계속 새로운 객체가 생성된다
		// StringBuffer : 가변성 보장
		 	// 변경 사항이 있더라도 하나의 객체만 사용
		...
		try {
			int result = service.insertComment(boardNo, sb.toString(), Session.loginMember.getMemberNo());
									// 현재 게시글 번호, 등록할 댓글내용, 로그인한 사람의 회원번호
	...
	public void deleteComment(int boardNo) {
		...
		try {
			...
			// 입력한 댓글 번호가 해당 게시글에 대해 로그인한 회원이 작성한 댓글이 맞는지 확인
			int check = service.checkCommentNo(deleteNo, boardNo, Session.loginMember.getMemberNo());
			...
			int result = service.deleteComment(deleteNo);
